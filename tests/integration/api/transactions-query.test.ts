/**
 * API Integration Tests for Transactions Query
 *
 * Tests the GraphQL transactions query (list with filters and pagination).
 *
 * Run with: bun test tests/integration/api/transactions-query.test.ts
 */
...
describe('Query: transactions', () => {
  ...
  test('should filter by category', async () => {
    ...
    expect(result.errors).toBeUndefined();
    expect(result.data?.transactions.items).toHaveLength(1);
    expect(result.data?.transactions.items[0]?.counterpartyName).toBe('Silpo');
  });
  ...
  test('should filter by type', async () => {
    ...
    expect(result.errors).toBeUndefined();
    expect(result.data?.transactions.items).toHaveLength(1);
    expect(result.data?.transactions.items[0]?.counterpartyName).toBe('Income');
  });
